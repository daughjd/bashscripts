...
class SkyMap:
    ...
    def __init__(self):
        ...
        self.sourcelist =  ""
        self.sourcenames = []
        self.sourcera = [248.491]
        self.sourcedec = [33.148]
        self.annotate = False
        self.vmax = 4.2
        self.hotspot = True
    ...
    def AddHistogramFile(self, file, ClockWise=False):
        ...
        if (f.IsOpen()):
            roothist = ROOT.TH2D(f.Get("hAllSkyCoarse"))
            roothistfine = ROOT.TH2D(f.Get("hAllSkyFine"))
        ...
        self.nbinsx = nx = roothistfine.GetNbinsX()
        self.nbinsy = ny = roothistfine.GetNbinsY()

        nxfine = roothistfine.GetNbinsX()
        nyfine = roothistfine.GetNbinsY()

        raLow = roothistfine.GetXaxis().GetBinLowEdge(1)
        raHigh = roothistfine.GetXaxis().GetBinLowEdge(nxfine + 1)
        
        decLow = roothistfine.GetYaxis().GetBinLowEdge(1)
        decHigh =  roothistfine.GetYaxis().GetBinLowEdge(nyfine + 1)
        
        
        roothistfineNorth = ROOT.TH2D("hAllSkyFineNorth", "", nxfine, raLow, raHigh , nyfine, decLow, decHigh )
        roothistfineSouth = ROOT.TH2D("hAllSkyFineSouth", "", nxfine, raLow, raHigh , nyfine, decLow, decHigh )
        

        #Fill north and south histograms


        for binx in range(1, nxfine +1):
            for biny in range(1, nyfine +1):
                c = roothistfine.GetBinContent(binx, biny)

                dec = roothistfine.GetYaxis().GetBinCenter(biny)
                
                if dec < -5:
                    roothistfineNorth.SetBinContent(binx, biny, 0)
                    roothistfineSouth.SetBinContent(binx, biny, c)
                else:
                    roothistfineNorth.SetBinContent(binx, biny, c)
                    roothistfineSouth.SetBinContent(binx, biny, 0)
        ...
        self.maxval = roothistfine.GetBinContent(maxbin)
        i, j = ROOT.Long(0), ROOT.Long(0)
        
        roothistfine.GetBinWithContent2(self.maxval, i, j, 1, 3600, 1, 1700, 0.01)

        self.ra_max = roothistfine.GetXaxis().GetBinCenter(i)
        self.dec_max = roothistfine.GetYaxis().GetBinCenter(j)
        
        
        maxbinNorth = roothistfineNorth.GetMaximumBin()
        self.maxvalN = roothistfineNorth.GetBinContent(maxbinNorth)
        
        roothistfineNorth.GetBinWithContent2(self.maxvalN, i, j, 1, 3600, 1, 1700, 0.01)

        self.ra_maxN = roothistfineNorth.GetXaxis().GetBinCenter(i)
        self.dec_maxN = roothistfineNorth.GetYaxis().GetBinCenter(j)
        


        
        
        maxbinSouth = roothistfineSouth.GetMaximumBin()
        self.maxvalS = roothistfineSouth.GetBinContent(maxbinSouth)
        
        roothistfineSouth.GetBinWithContent2(self.maxvalS, i, j, 1, 3600, 1, 1700, 0.01)

        self.ra_maxS = roothistfineSouth.GetXaxis().GetBinCenter(i)
        self.dec_maxS = roothistfineSouth.GetYaxis().GetBinCenter(j)
        ...
        self.ub = underbins = int((ymin + 90) / binwidth)
        self.ob = overbins = int((90 - ymax) / binwidth)
        ...
        self.content = array([array([roothistfine.GetBinContent(i, j)
        ...
        self.xedges = array([roothistfine.GetXaxis().GetBinLowEdge(i)
                             for i in range(1, nx + 2)])
        
        
        self.yedges = array([roothistfine.GetYaxis().GetBinLowEdge(i)
    ...
    def Plot(self, proj, clockwise=False):
        ...
        if (len(self.evxcoord) is not 0):
            self.content = array([array([0
                                  ...
                                  for j in range(1 - self.ub, self.nbinsy + 1 + self.ob)])
        ...
        if(proj == "rectangular"):
            ...
            plt.set_yticklabels(ylabels, fontsize=15, family='sans serif', weight='bold')
        ...
        if self.hotspot == True:
            
            if proj == "rectangular":
                plt.scatter(self.ra_maxN , self.dec_maxN, lw=2, facecolor='none', marker='o', s=60)
                #plt.scatter(self.ra_maxS , self.dec_maxS, lw=2, facecolor='none', marker='o', s=60)

            else:
            
                if clockwise == False:
                
                    plt.scatter((360 - self.ra_maxN - 180.0) * np.pi/180, self.dec_maxN * np.pi/180, lw=2, facecolor='none', marker='o', s=160)
                    #plt.scatter((360 - self.ra_maxS - 180.0) * np.pi/180, self.dec_maxS * np.pi/180, lw=2, facecolor='none', marker='o', s=60)

                else:
                    plt.scatter((self.ra_maxN - 180.0) * np.pi/180, self.dec_maxN * np.pi/180, lw=2, facecolor='none', marker='o', s=80)
                    #plt.scatter((self.ra_maxS - 180.0) * np.pi/180, self.dec_maxS * np.pi/180, lw=2, facecolor='none', marker='o', s=60)
        ...
        print "Fine Grid Hottest Spot in the North Sky:"
        print "-log10(p) = ",self.maxvalN
        print "Ra: ",self.ra_maxN
        print "Dec:",self.dec_maxN        


        print "Fine Grid Hottest Spot in the South Sky:"
        print "-log10(p) = ",self.maxvalS
        print "Ra: ",self.ra_maxS
        print "Dec:",self.dec_maxS        
